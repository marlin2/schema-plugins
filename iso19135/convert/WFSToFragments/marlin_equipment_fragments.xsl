...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="wfs:FeatureCollection">
		<records>
			...
			<xsl:variable name="uuid" select="'urn:marlin.csiro.au:Equipment'"/>
	...
	<!-- process a record from the MarLIN equipment table -->
	<xsl:template name="addEquipmentRegisterItem">
		<xsl:param name="keywordUuid"/>

		<fragment id="register_item" uuid="{$keywordUuid}" title="{normalize-space(app:oa_equipment_label)}">
			<grg:containedItem>
				<grg:RE_RegisterItem>
					<grg:itemIdentifier>
						<gco:Integer><xsl:value-of select="app:oa_equipment_id"/></gco:Integer>
					</grg:itemIdentifier>
					<grg:name>
						<gco:CharacterString><xsl:value-of select="app:oa_equipment_label"/></gco:CharacterString>
					</grg:name>
					<grg:status>
	...
	<xsl:template match="gml:featureMember">
		...
		<xsl:apply-templates select="app:Equipment">
			<xsl:with-param name="uuid" select="$uuid"/>
		</xsl:apply-templates>
	...
	<xsl:template match="app:Equipment">
			<xsl:param name="uuid"/>

			<xsl:call-template name="addEquipmentRegisterItem">
				<xsl:with-param name="keywordUuid" select="concat($uuid,':concept:',app:oa_equipment_id)"/>
			</xsl:call-template>

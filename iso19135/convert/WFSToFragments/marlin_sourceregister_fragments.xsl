...
<xsl:stylesheet version="2.0"
	...
	<!-- process a record from the MarLIN sources table -->
	...
	<xsl:template match="gml:featureMember">
		...
		<xsl:apply-templates select="app:MarlinSources">
			<xsl:with-param name="uuid" select="$uuid"/>
		</xsl:apply-templates>

	</xsl:template>

	<!-- process the MarlinSources in WFS response -->
	<xsl:template match="app:MarlinSources">
			<xsl:param name="uuid"/>

			<xsl:call-template name="addSourceRegisterItem">
				<xsl:with-param name="keywordUuid" select="concat($uuid,':concept:',app:source_id)"/>
			</xsl:call-template>
